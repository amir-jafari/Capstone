...
env = DriftingEnvironment(NUM_USERS, NUM_DOCUMENTS,ALPHA,A,B,BETA1,BETA2, CATEGORIES, SEED)
...
# print(users[0])
# print(documents)
# print(documents[0].topic_distribution.sum())
#%%
...
for round_index in range(NUM_ROUNDS):        
    ...
    for user_index, user in enumerate(users):
        ...
        random_model_rewards.append(cumulative_reward_random)
...
for round_index in range(NUM_ROUNDS):        
    ...
    for user_index, user in enumerate(users):
        ...
        users[user_index] = updated_user  
        print()           
        cumulative_reward_tdq += reward
        q_learning_model_rewards.append(cumulative_reward_tdq)
        latent_preference_list_tdq.append(updated_user.theta.copy())
        model.update(action, user.theta, updated_user.theta, reward)



# %%

# plot 1 - visulization of user latent preferences (plot where x axis is your time and y axis is your latent score)
# this visualization should add up to 1 the latent score (make sure this is true)
# this plot should show the user preference drifting over time.

# if you have any questions about this please reach out to me.

latent_preferences = np.array(latent_preference_list_tdq) 
print(latent_preferences.sum(axis=1)) 
plt.figure(figsize=(10, 6))
for category_index in range(latent_preferences.shape[1]):
    plt.plot(latent_preferences[:, category_index], label=f'Category {category_index}')
...
# plot 2 - model benchmarking (cumulative sum of rewards over time)
# plot #2.1 (random choice model -> np.random.choice(documents))
# plot #2.2 (q-learning model)

plt.figure(figsize=(10, 6))
plt.plot(random_model_rewards, label='Random Model')
plt.plot(q_learning_model_rewards, label='TDQ Model')
plt.xlabel('Time (Rounds)')
plt.ylabel('Cumulative Reward')
plt.title('Model Benchmarking: Cumulative Reward Over Time')
plt.legend()
plt.grid(True)
plt.show()

# if your random model is doing better than your q-learning model then you have a bug in your code.


# %%

plt.figure(figsize=(10, 6))
# plt.bar(height=documents[0].x, label='Document 0 Topic Distribution')
plt.bar(range(len(documents[1].x)), height=documents[1].x)
plt.xlabel('Categories')
plt.ylabel('Topic Distribution')
plt.title('Document 0 Topic Distribution')
plt.show()
# plt.plot(documents[1].topic_distribution, label='Document 1 Topic Distribution')
# %%

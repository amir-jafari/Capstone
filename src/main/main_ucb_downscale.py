...
from src.component.model_ucb import *
import numpy as np
...
ucb_model = UCB(n_arms=NUM_DOCUMENTS, random_seed=SEED)

# Store cumulative rewards per episode
ucb_cum_rew_list = []
...
for episode in range(EPISODES):
    ...
    if episode%10 == 0:
        ...
        for category_index in range(num_categories):
            ...
            plt.plot(avg_latent_prefs_tdq[:, category_index], label=f'TDQ {CATEGORIES[category_index]}',color = color[category_index])
...
for episode in range(EPISODES):
    ...
    for step in range(NUM_ROUNDS):
        for user_index, user in enumerate(users):
            # Use UCB to select action/document
            action = ucb_model.action()
            selected_document = documents[action]

            # Take environment step, obtain reward and updated user
            reward, updated_user = env.step(user, selected_document)
            users[user_index] = updated_user

            # Update UCB model with action and reward
            scalar_reward = reward if np.isscalar(reward) else np.sum(reward)
            ucb_model.update(action, scalar_reward)

            ucb_cum_rew += scalar_reward
...
#%%
num_categories = avg_latent_prefs_tdq.shape[1]  # assuming same categories

for category_index in range(num_categories):
    plt.plot(avg_latent_prefs_random[:, category_index], linestyle='dashed', label=f'Random {CATEGORIES[category_index]}')
    plt.plot(avg_latent_prefs_tdq[:, category_index], label=f'TDQ {CATEGORIES[category_index]}', color=color[category_index])
    plt.plot(avg_latent_prefs_ucb[:, category_index], linestyle='dotted', label=f'UCB {CATEGORIES[category_index]}', color=color[category_index])

plt.xlabel('Time (Rounds)')
plt.ylabel('Average Latent Preference Score')
plt.title('Average Latent Preferences Over Episodes: Random vs TDQ vs UCB')

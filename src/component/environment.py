...
class DriftingDocumentSampler(recsim.document.AbstractDocumentSampler):
  ...
  def sample_document(self, doc_id):
    k = np.random.randint(1, self.num_categories + 1)
    category_idx = np.random.choice(self.num_categories, p=self.category_probs,size = k)
    x = np.zeros(self.num_categories, dtype=np.float32)
    x[category_idx] = 1.0
    p = np.random.beta(self.a, self.b)
    q = np.random.beta(self.b, self.a)
...
class DriftingUserState(recsim.user.AbstractUserState):
  ...
  def __init__(self, user_id, theta, sigma, phi, num_categories, tau=0):
    super(DriftingUserState, self).__init__(user_id)
    self.theta = theta
    self.sigma = sigma
    self.phi = phi
    self.tau = tau

...
class DriftingUserModel(recsim.user.AbstractUserModel):
    ...
    def update_state(self, user_state, doc):
        ...
        epsilon_sigma = self.rng.normal(0, 0.005)
        epsilon_phi = self.rng.normal(0, 0.005)
        ...
        user_state.sigma =  (self.alpha ** 2) * user_state.sigma + (1-self.alpha)**2 * doc.p + (2* self.alpha * (1-self.alpha)) * doc.q  #+ epsilon_sigma
        user_state.phi =  (self.alpha ** 2) * user_state.phi + (2* self.alpha * (1-self.alpha)) * doc.p  + (1-self.alpha)**2 * doc.q #+epsilon_phi
        return user_state
